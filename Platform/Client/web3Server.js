exports.newWeb3Server = function newWeb3Server() {
    ...
    let thisObject = {
        ...
        getUserLPBalance: getUserLPBalance,
    ...
    async function getUserWalletBalance(walletAddress, contractAddress) {
        ...
        let balance, reserveSa, totalSupply
    ...
    async function getUserLPBalance(walletAddress, contractAddress) {

        const bscUri = 'https://bsc-dataseed.binance.org/'
        const web3 = new Web3(bscUri)

        let balance, saBalance
        ...
        async function getBalance(){
            balance = await contractInst.methods.balanceOf(walletAddress).call().then(result => web3.utils.fromWei(result, 'ether'))
            reserveSa = await contractInst.methods.getReserves().call().then(result => web3.utils.fromWei(result[1], 'ether'))
            totalSupply = await contractInst.methods.totalSupply().call().then(result => web3.utils.fromWei(result, 'ether'))
            //saBalance = ((balance / totalSupply) * reserveSa).toString()
            //return saBalance, balance
        }

        await getBalance().then( () => saBalance = ((balance / totalSupply) * reserveSa).toString())
